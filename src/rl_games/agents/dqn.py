...
class QNetwork(nn.Module):
    """
    Arquitectura de Red Neuronal para DQN.
    Mapea un vector de estado de dimensión N a valores Q para cada acción.
    """
    def __init__(self, state_dim: int, action_dim: int, hidden_dim: int = 128) -> None:
        super().__init__()
        
        self.network = nn.Sequential(
            # Primera Capa: Entrada -> Oculta 1
            nn.Linear(state_dim, hidden_dim),
            nn.ReLU(),
            
            # Segunda Capa: Oculta 1 -> Oculta 2
...
class DQNAgent:
    ...
    def _encode_state(self, obs: int) -> np.ndarray:
        """Convierte posición entera a vector one-hot."""
    ...
    def predict(self, obs, *, deterministic=True):
        return self.select_action(self._encode_state(obs), deterministic=deterministic), None
    ...
    def train(self, total_episodes: int = 500, log_interval: int = 10) -> list[float]:
        ...
        for episode in range(1, total_episodes + 1):
            ...
            while not done:
                # Vectorize obs
                obs_encoded = self._encode_state(obs)
                # Select action
                action = self.select_action(obs_encoded)
                # Take action
                next_obs, reward, terminated, truncated, _ = env.step(action)
                # Vectorize obs
                next_obs_encoded = self._encode_state(next_obs)
                # Update state
                done = terminated or truncated
                # Update buffer
                self.buffer.push(obs_encoded, action, float(reward), next_obs_encoded, done)
                # Update Q-network
                self._learn()
